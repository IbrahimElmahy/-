var WebServices = function () {
    ...
    const $order = $('#order');
    const $rent = $('#rent')
    // Table for the print view (in print.html)
    ...
    // FUNCTIONS FOR THE PRINT MODE
    // ===============================
     // -----------------------------
    // PRINT MODE: renderPrintTable()
    // -----------------------------
    var renderPrintTable = function () {
        if (!dataTablePrintElement.length) {
            return 0;
        }
        // Get the parent's search form serialization.
        var $document = $(parent.document);
        var $form = $document.find('form');
        var formQS = $form.serialize(); // these are the filter values

        // (Optionally) Also grab any DataTable paging/search parameters:
        var dt = dataTableElement.DataTable();
        var tableParams = dt.ajax ? dt.ajax.params() : {};
        var tableQS = $.param(tableParams);

        // Merge the two query strings. (If both exist, join with '&'.)
        var queryString = formQS;
        if (tableQS) {
            queryString = queryString ? queryString + '&' + tableQS : tableQS;
        }

        // Build the API URL using the form's data-url attribute.
        var baseUrl = $form.data('url');
        // Append is_export=true and our merged query string.
        var url = baseUrl + '?is_export=true&' + queryString;
        console.log("Daily print URL:", url);

        // Create a local DataTable for the print table.
        var table = Table.renderLocalTableView(dataTablePrintElement);

        // Make the AJAX GET call.
        Ajax.get(url, {}, false, function (response) {
            // IMPORTANT: Our API returns an array at the root.
            $.each(response, function (key, item) {
                table.row.add([
                    key + 1,                                 // Serial #
                    item['number'],                          // Reservation Number
                    item['guest'],                           // Guest Name
                    item['apartment'],                       // Apartment
                    item['check_in_date'],                   // Checked in
                    item['check_out_date'],                  // Checked out
                    item['time'],                            // Time
                    item['status_display'],                  // Reservation Status
                    item['rental_display'],                  // Rental Type
                    item['period'],                          // Period
                    item['rent'],                            // Rent
                    item['amount'],                          // Amount
                    item['discount_display'],                // Discount
                    item['subtotal'],                        // Subtotal
                    item['tax'],                             // Tax
                    item['total'],                           // Total
                    item['paid'],                            // Paid
                    item['balance']                        // Balance
                    // Table.renderDate(item['created_at']),    // Created At (formatted)
                    // Table.renderDate(item['updated_at'])     // Updated At (formatted)
                ]).draw(false);
            });
            // Optionally, if you no longer need the temporary table instance:
            table.destroy();
        });
    };
    ...
    return {
        ...
        init: function () {
            ...
            renderEventListener();
            actionFilterObject();
            // If the print table element exists (i.e. we're in print mode), render it.
            renderPrintTable();
