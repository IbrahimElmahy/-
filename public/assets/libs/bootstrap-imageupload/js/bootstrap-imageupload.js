...
(function($) {
    ...
    $.fn.imageupload = function(methodOrOptions) {
        ...
        return this.filter('div').each(function() {
            if (methods[methodOrOptions]) {
                ...
                throw new Error('طريقة "' + methodOrOptions + '"لم يتم تعريفه لتحميل الصور.');
            }
        });
    };
    ...
    function isValidImageFile(file, callback) {
        ...
        {
            callback(false, 'الملف كبير جدا (اعلا حجم ' + options.maxFileSizeKb + 'kB).');
            return;
        }
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            callback(true, 'ملف الصورة صالح.');
        }
        else {
            callback(false, 'نوع الملف غير مسموح به.');
        }
    }
    ...
    function isValidImageUrl(url, callback) {
        ...
        image.onload = function() {
            if (!timeout) {
                ...
                if ($.inArray(fileExtension, options.allowedFormats) > -1) {
                    callback(true, 'عنوان URL للصورة صالح.');
                }
                else {
                    callback(false, 'نوع الملف غير مسموح به.');
                }
        ...
        image.onerror = function() {
            if (!timeout) {
                window.clearTimeout(timer);
                callback(false, 'لم يتم العثور على صورة.');
            }
        };
        ...
        timer = window.setTimeout(function() {
            ...
            callback(false, 'انتهت مهلة تحميل الصورة.');
        }, timeoutMs);
    }
    ...
    function resetFileTab($fileTab) {
        ...
        $fileTab.find('.btn span').text('تصفح الصور');
    ...
    function submitImageFile($fileTab) {
        ...
        $fileTab.find('.alert').remove();
        $fileTab.find('img').remove();
        $browseFileButton.find('span').text('تصفح الصور');
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onload = function(e) {
                    ...
                    $browseFileButton.find('span').text('تعديل');
                    $removeFileButton.css('display', 'inline-block');
                };

                fileReader.onerror = function() {
                    $fileTab.prepend(getAlertHtml('خطأ في تحميل ملف الصورة.'));
                    $fileInput.val('');
                };
    ...
    function submitImageUrl($urlTab) {
        ...
        if (!url) {
            $urlTab.prepend(getAlertHtml('الرجاء إدخال عنوان URL للصورة.'));
            return;
        }
